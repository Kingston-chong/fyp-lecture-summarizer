...
function findRangeForSubstring(root, quote) {
  ...
  const big = nodes.map((node) => node.textContent ?? "").join("").replace(/\s+/g, " ");
...
export function wrapQuoteInRoot(root, quote, hlId, colorHex, pending) {
  ...
  try {
    ...
    // Reset background on any child elements (e.g. <strong>, <em>, <h2>, <li>)
    // so the mark's background color shows through uniformly instead of only
    // on bare text nodes.
