 ...
 * @param {{ markdown?: string, headings?: { text: string }[], title?: string | null, max?: number }} opts
 * @returns {string[]}
 */
export function buildChatSuggestions({
  ...
  // No / few headings: derive a snippet from body text
  const bodySnippet = extractBodySnippet(md);
  if (bodySnippet) {
    const sn = clipTopic(bodySnippet);
    push(`Summarize the main idea of this part in one short paragraph: ${sn}`);
