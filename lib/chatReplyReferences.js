...
function hasBrokenReferenceTitle(title) {
  ...
  if (/\.(pdf|docx?|pptx?)\)?$/i.test(t) && !/\]\(https?:\/\//i.test(t)) return true;
...
export function finalizeChatReplyReferences(
  ...
  let refsForSection = chatSources.filter((s) => {
    if (!isUsableReferenceSource(s, summaryTitle, { articlesOnly })) return false;
    if (citedMarkers.size > 0 && citedMarkers.has(Number(s.marker))) return true;
