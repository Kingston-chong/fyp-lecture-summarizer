...
function ensureContentTypeDefault(contentTypesXml, ext, contentType) {
  const exists = new RegExp(
    `<Default\\s+Extension="${ext}"\\s+ContentType="[^"]+"\\s*\\/?>`,
    "i",
  );
...
function estimateTextBounds(slideXml, dims) {
  ...
  const xfrmRe =
    /<a:off\s+x="(\d+)"\s+y="(\d+)"\s*\/>[\s\S]*?<a:ext\s+cx="(\d+)"\s+cy="(\d+)"\s*\/>/g;
  let maxRight = 0;
  let maxBottom = 0;
  let m;
...
/**
 * Shrink any <p:sp> text-box shapes whose bounding box overlaps the image area.
 *
 * For each overlapping text shape:
 *  - If the image is on the RIGHT side of the slide (ix1 > sw/2):
 *    clip the shape's width so its right edge = ix1 - gap
 *  - If the image is on the LEFT side (ix1 <= sw/2):
 *    move the shape's left edge to ix2 + gap and shrink width accordingly
 *  - If the image is on the BOTTOM (iy1 > sh/2):
 *    clip the shape's height so its bottom edge = iy1 - gap
 *  - If the image is on the TOP (iy1 <= sh/2):
 *    move the shape's top edge to iy2 + gap and shrink height accordingly
 *
 * Only shrinks; never expands a shape beyond its original size.
 *
 * @param {string} slideXml
 * @param {{ xIn: number; yIn: number; wIn: number; hIn: number }} imgPos
 * @param {{ wIn: number; hIn: number }} dims
 * @returns {string} patched slideXml
 */
function shrinkOverlappingTextBoxes(slideXml, imgPos, dims) {
  const GAP = 0.12; // inches gap between text and image
  const { xIn: ix1, yIn: iy1, wIn: iw, hIn: ih } = imgPos;
  const ix2 = ix1 + iw;
  const iy2 = iy1 + ih;
  const { wIn: sw, hIn: sh } = dims;

  // Determine which side the image is on
  const imageOnRight = ix1 > sw / 2;
  const imageOnTop = iy1 < sh / 2;

  // Match each <p:sp>…</p:sp> block that contains a <a:xfrm> with off+ext
  return slideXml.replace(/<p:sp\b[\s\S]*?<\/p:sp>/g, (spBlock) => {
    // Only text shapes (those with <a:txBody>)
    if (!spBlock.includes("<a:txBody")) return spBlock;

    const xfrmMatch = spBlock.match(
      /(<a:xfrm\b[^>]*>)\s*<a:off\s+x="(\d+)"\s+y="(\d+)"\s*\/>\s*<a:ext\s+cx="(\d+)"\s+cy="(\d+)"\s*\/>/,
    );
    if (!xfrmMatch) return spBlock;

    const sx1 = Number.parseInt(xfrmMatch[2], 10) / EMU;
    const sy1 = Number.parseInt(xfrmMatch[3], 10) / EMU;
    const scx = Number.parseInt(xfrmMatch[4], 10) / EMU;
    const scy = Number.parseInt(xfrmMatch[5], 10) / EMU;
    const sx2 = sx1 + scx;
    const sy2 = sy1 + scy;

    // Check if shape overlaps with image region (with a small tolerance)
    const overlapX = sx1 < ix2 - 0.05 && sx2 > ix1 + 0.05;
    const overlapY = sy1 < iy2 - 0.05 && sy2 > iy1 + 0.05;
    if (!overlapX || !overlapY) return spBlock;
    ...
    if (imageOnRight) {
      // Clip the shape's right edge to just before the image
      const newRight = ix1 - GAP;
      if (newRight < sx1 + 0.3) return spBlock; // would be too narrow, skip
      newCx = newRight - sx1;
    } else {
      // Push the shape's left edge past the image
      const newLeft = ix2 + GAP;
      if (newLeft > sx2 - 0.3) return spBlock; // would be too narrow, skip
      newCx = sx2 - newLeft;
      newX = newLeft;
    }

    if (imageOnTop && overlapY && !imageOnRight) {
      // Also clip vertical if image is top-left and shape overlaps vertically
      const newBottom = iy2 + GAP;
      if (newBottom > sy2 - 0.2) return spBlock;
      newY = newBottom;
      newCy = sy2 - newBottom;
    } else if (!imageOnTop && overlapY && !imageOnRight) {
      // Clip bottom
      const newBottom = iy1 - GAP;
      if (newBottom < sy1 + 0.2) return spBlock;
      newCy = newBottom - sy1;
    }

    const newXEmu = Math.round(newX * EMU);
    const newYEmu = Math.round(newY * EMU);
    const newCxEmu = Math.round(newCx * EMU);
    const newCyEmu = Math.round(newCy * EMU);

    return spBlock.replace(
      /<a:off\s+x="\d+"\s+y="\d+"\s*\/>\s*<a:ext\s+cx="\d+"\s+cy="\d+"\s*\/>/,
      `<a:off x="${newXEmu}" y="${newYEmu}"/><a:ext cx="${newCxEmu}" cy="${newCyEmu}"/>`,
    );
  });
}
...
export async function buildPptxWithImagesInPlace(sourceBuffer, images) {
  ...
  let imageNum = nextImageNum(
    files.filter((p) => /^ppt\/media\/image\d+\.[a-z0-9]+$/i.test(p)),
  );

  const contentTypesPath = "[Content_Types].xml";
  let contentTypesXml = await zip.file(contentTypesPath)?.async("string");
  if (!contentTypesXml)
    throw new Error("Invalid PPTX package: missing [Content_Types].xml");
  ...
  for (const im of images || []) {
    ...
    const corner = chooseImageCorner(slideXml, dims, {
      wIn: IMG_W_IN,
      hIn: IMG_H_IN,
    });
    ...
    // Shrink any text boxes that overlap the image placement area
    const patchedSlideXml = shrinkOverlappingTextBoxes(
      slideXml,
      { xIn: corner.xIn, yIn: corner.yIn, wIn: IMG_W_IN, hIn: IMG_H_IN },
      dims,
    );

    zip.file(
      relsPath,
      appendRelationship(relsXml, relId, `../media/${mediaName}`),
    );
    zip.file(
      slidePath,
      patchedSlideXml.replace(/<\/p:spTree>/i, `${picXml}\n  </p:spTree>`),
    );
  }
