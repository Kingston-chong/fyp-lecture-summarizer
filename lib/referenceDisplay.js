...
export function syntheticUploadReference(marker, files) {
  ...
  if (f.sourceUrl) {
    return {
      ...
      kind: "web",
      title: String(f.name || "Web page").trim(),
      authors: null,
      year: null,
      venue: null,
      doi: null,
 ...
 */
/**
 * Wikipedia-style ^ a b back-links when a source is cited more than once.
 * @param {string[]} anchorIds
 */
export function buildCitationBacklinksHtml(anchorIds) {
  if (!Array.isArray(anchorIds) || anchorIds.length <= 1) return "";
  const links = anchorIds
    .map((id, i) => {
      const letter = citationLetterForIndex(i);
      const safeId = escapeHtmlAttr(String(id));
      return `<a href="#${safeId}" class="ref-cite-backlink">${escapeHtml(letter)}</a>`;
    })
    .join(" ");
  return `<span class="ref-cite-backlinks" aria-label="Jump to citations in summary"><span class="ref-cite-caret" aria-hidden="true">^</span> ${links}</span>`;
}
...
export function buildReferencesSectionHtml(references) {
  ...
  const items = references
    .map((ref) => {
      ...
      const backlinks = buildCitationBacklinksHtml(ref.anchorIds);

      return `<li id="ref-${marker}" class="ref-biblio-item" data-marker="${marker}">
        <span class="ref-biblio-marker">[${marker}]${backlinks ? ` ${backlinks}` : ""}</span>
