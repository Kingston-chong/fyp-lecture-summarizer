...
export function allowedExtensionFromContentType(contentType) {
  ...
  return ALLOWED_EXTENSIONS.has(mapped.toUpperCase()) ? mapped : null;
}

/**
 * HEAD probe — true when the URL responds with a supported document Content-Type.
 * @param {string} sourceUrl
 */
export async function probeRemoteDocumentContentType(sourceUrl) {
  ...
  try {
    ...
    return allowedExtensionFromContentType(res.headers.get("content-type"));
  } catch {
    return null;
  }
}
