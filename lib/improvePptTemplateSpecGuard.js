 ...
 */
export function selectSafeTemplateSpec({
  incomingSpec,
  resolvedSpec,
  builtinFallback,
}) {
  const incoming = normalizeTemplateSpec(incomingSpec);
  if (incoming.ok && incoming.spec)
    return { spec: incoming.spec, source: "incoming" };

  const resolved = normalizeTemplateSpec(resolvedSpec);
  if (resolved.ok && resolved.spec)
    return { spec: resolved.spec, source: "resolved" };

  const builtin = normalizeTemplateSpec(builtinFallback);
  if (builtin.ok && builtin.spec)
    return { spec: builtin.spec, source: "builtin_fallback" };
