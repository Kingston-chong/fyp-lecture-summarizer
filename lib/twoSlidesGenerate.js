...
/**
 * Submit async presentation generation (2slides presentations API).
...
export async function submitTwoSlidesGeneration({ inputText, themeId, title }) {
  ...
  const body = {
    ...
    ...(themeId ? { theme_id: String(themeId).trim() } : {}),
    ...(title ? { options: { title: String(title).trim() } } : {}),
  };
...
/**
 * Poll presentation generation status.
