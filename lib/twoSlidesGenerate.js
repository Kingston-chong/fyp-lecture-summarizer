...
async function fetchJson(url, init = {}) {
  const { res, data } = await twoSlidesFetch(url, init);
  if (!res.ok) {
    const msg =
      extractTwoSlidesMessage(data) ||
      `HTTP ${res.status}`;
    throw new Error(msg);
  }
...
export function improvedSlidesToTwoSlidesUserInput({
  ...
  if (instr) {
    lines.push("");
    lines.push("User improvement goals:");
    lines.push(instr);
  }
  lines.push("");
 ...
 * Documented params: themeId, userInput, responseLanguage, mode.
 * @param {{ inputText: string, themeId: string, responseLanguage?: string }} opts
 */
export async function submitTwoSlidesGeneration({
  ...
  if (!theme) {
    return {
      ok: false,
      error:
        "A theme is required for 2slides. Search and select a theme first.",
    };
  }
  ...
  const { res, data } = await twoSlidesFetch(
    `${TWOSLIDES_BASE}/api/v1/slides/generate`,
    {
      method: "POST",
  ...
  if (!res.ok) {
    return {
      ok: false,
      error: extractTwoSlidesMessage(data) || `2slides error ${res.status}`,
    };
  }
...
export async function pollTwoSlidesGeneration(jobId) {
  ...
  const { res, data } = await twoSlidesFetch(jobUrl.toString(), {
    cache: "no-store",
    signal: AbortSignal.timeout(20_000),
  });
  ...
  if (!res.ok) {
    return {
      status: "failed",
      error:
        extractTwoSlidesMessage(data) || `2slides poll error ${res.status}`,
    };
  }
