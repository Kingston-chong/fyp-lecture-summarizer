...
import {
  GUEST_MAX_FILES,
  parseGuestSummarizeFormData,
  validateGuestFileSelection,
} from "./guestUpload.js";
import { SERVERLESS_PAYLOAD_MAX_BYTES } from "./uploadLimits.js";

test("parseGuestSummarizeFormData rejects empty form", async () => {
  const fd = new FormData();
  fd.append("model", "chatgpt");
  await assert.rejects(() => parseGuestSummarizeFormData(fd), /file/i);
});
...
test("validateGuestFileSelection rejects oversized total", () => {
  const half = Math.floor(SERVERLESS_PAYLOAD_MAX_BYTES / 2) + 1;
  const result = validateGuestFileSelection([
...
test("parseGuestSummarizeFormData enforces max file count", async () => {
  ...
  await assert.rejects(() => parseGuestSummarizeFormData(fd), /up to/i);
});
