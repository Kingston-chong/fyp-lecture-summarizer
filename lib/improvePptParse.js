...
import { enrichSlidesWithOcr } from "@/lib/ocrSlides";

export const MAX_SLIDE_TEXT = 4000;
export const MAX_IMPROVE_PDF_PAGES = 50;

/**
 * @typedef {{ ocr?: boolean; modelKey?: string; modelVariant?: string | null }} ParseSlideOpts
 */
...
/**
 * Structural parse plus optional OCR enrichment (for parse API).
 * @param {Buffer} buffer
 * @param {boolean} isPdf
 * @param {ParseSlideOpts} [opts]
 */
export async function parseBufferToSlidesWithMeta(buffer, isPdf, opts = {}) {
  const slides = isPdf
    ? await parsePdfBufferToSlides(buffer)
    : await parsePptxBufferToSlides(buffer);

  if (!opts.ocr) {
    return { slides, ocrApplied: 0 };
  }

  const result = await enrichSlidesWithOcr(slides, buffer, {
    isPdf,
    modelKey: opts.modelKey || "gemini",
    modelVariant: opts.modelVariant ?? null,
  });

  return {
    slides: truncateSlides(result.slides),
    ocrApplied: result.ocrApplied,
    ...(result.ocrWarning ? { ocrWarning: result.ocrWarning } : {}),
  };
}
