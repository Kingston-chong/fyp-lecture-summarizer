...
function buildClrSchemeXml(theme) {
  ...
  const accent4 = darken(acc, 0.2);
  const accent5 = mix(acc, bg, 0.35);
  const accent6 = mix(acc, txt, 0.3);
  ...
  if (pan) {
    ...
    const shifted =
      lum < 128
        ...
        : darken(bg, 0.1); // light bg → shift toward black
    lt2 = toHex6(...Object.values(shifted).map(Math.round));
  }

  return (
    `<a:clrScheme name="Slide2Notes Theme">` +
...
function patchShapeFills(slideXml, oldTheme, newTheme, threshold = 60) {
  ...
  const recolorPart = (part) =>
    part.replace(
      /<a:srgbClr\s+val="([0-9A-Fa-f]{6})"\s*\/>/g,
      (match, hexVal) => {
  ...
  return parts
    .map((part, i) => {
      ...
      if (i % 2 === 1)
        return part.replace(/\x00TC_START\x00|\x00TC_END\x00/g, "");
...
function buildExplicitTableStylesXml(styleIds, theme) {
  ...
  function bestTextOn(bg_c) {
    ...
    return contrastRatio(white, bg_c) >= contrastRatio(black, bg_c)
      ? white
      : black;
  }
  ...
  // ── XML builders ─────────────────────────────────────────────────────────────
  const solid = (c) =>
    `<a:solidFill><a:srgbClr val="${hex6(c)}"/></a:solidFill>`;
...
async function collectTableStyleIds(zip, slidePaths) {
  ...
  for (const path of slidePaths) {
    ...
    for (const m of xml.matchAll(
      /<a:tableStyleId>(\{[^}]+\})<\/a:tableStyleId>/g,
    )) {
      ids.add(m[1]);
    }
  }
...
export async function applyThemeToPptx(sourceBuffer, newTheme) {
  ...
  const themePaths = Object.keys(zip.files).filter((p) =>
    /^ppt\/theme\/theme\d*\.xml$/i.test(p),
  );
  ...
  const layoutPaths = Object.keys(zip.files).filter((p) =>
    /^ppt\/slideLayouts\/slideLayout\d*\.xml$/i.test(p),
  );
  ...
  const explicitTableStyles = buildExplicitTableStylesXml(
    tableStyleIds,
    newTheme,
  );
...
export async function extractThemeFromPptx(sourceBuffer) {
  try {
    ...
    const shifted =
      lum < 128
        ? {
            r: bg.r + (255 - bg.r) * 0.14,
            g: bg.g + (255 - bg.g) * 0.14,
            b: bg.b + (255 - bg.b) * 0.14,
          }
