...
export async function* callAIStream(model, modelVariant, systemPrompt, documentText) {
  const clipped = (documentText || "").slice(0, MAX_MODEL_INPUT_CHARS);
  const fullPrompt = `${systemPrompt}\n\n---\n\nDocument Content:\n${clipped}`;
...
export function finalizeSummarizeOutput(raw, { isLecturer, referenceCatalog, referenceCatalogMeta }) {
...
export async function prepareSummarizeContextFromBuffers(params) {
  const { extractDocumentTextFromBuffer } = await import("@/lib/extractDocumentText");
