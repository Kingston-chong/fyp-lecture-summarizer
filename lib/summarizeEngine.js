...
import { summarizeLengthInstruction } from "@/lib/summarizeOutputLength";
import {
  buildProviderFallbackChain,
  isRetriableLlmError,
} from "@/lib/llmRetry";
import { getAvailableLlmProviders } from "@/lib/llmProviders";
...
async function callOpenRouter(model, modelVariant, fullPrompt, maxTokens = 2000) {
  ...
  for (const slug of slugs) {
    try {
      ...
      if (
        model === "gemini" &&
        (isOpenRouterModelNotFound(err) || isRetriableLlmError(err))
      )
        continue;
      throw err;
    }
  ...
  if (model === "gemini") {
    ...
    for (const modelName of uniqueCandidates) {
      try {
        ...
        lastErr = err;
        if (isRetriableLlmError(err)) continue;
        throw err;
      }
    }
...
/**
 * @param {{ maxTokens?: number; onProviderSwitch?: (info: { from: string; to: string }) => void }} [opts]
 */
export async function callAIWithFallback(
  ...
  const chain = buildProviderFallbackChain(model, getAvailableLlmProviders);
  let lastErr;

  for (let i = 0; i < chain.length; i++) {
    const provider = chain[i];
    const variant = provider === model ? modelVariant : null;
    try {
      return await callAI(
        provider,
        variant,
      ...
      if (!hasNext || !isRetriableLlmError(err)) throw err;
      opts.onProviderSwitch?.({ from: provider, to: chain[i + 1] });
      console.warn(
        `Summarize ${provider} failed (${err?.message || err}); switching to ${chain[i + 1]}…`,
      );
    }
  }
 ...
 * @param {{ maxTokens?: number; onProviderSwitch?: (info: { from: string; to: string }) => void }} [opts]
 */
export async function* callAIStreamWithFallback(
  model,
  modelVariant,
  systemPrompt,
  documentText,
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    try {
      ...
      lastErr = err;
      const hasNext = i < chain.length - 1;
      if (emitted || !hasNext || !isRetriableLlmError(err)) throw err;
      opts.onProviderSwitch?.({ from: provider, to: chain[i + 1] });
      console.warn(
        `Summarize stream ${provider} failed (${err?.message || err}); switching to ${chain[i + 1]}…`,
      );
    }
  }

  throw lastErr ?? new Error("Summarization stream failed");
}
 ...
 *   onStatus?: (payload: { phase: string; step?: string; queries?: string[]; paperCount?: number; papers?: { title: string; year?: number | null }[] }) => void;
 * }} params
 */
export async function prepareSummarizeContext({
  ...
  onStatus?.({ phase: SUMMARIZE_PHASE.EXTRACTING });
  ...
  if (isLecturer) {
    try {
      const papers = await fetchRelatedPapers(
        ...
        (refDetail) =>
          onStatus?.({
            phase: SUMMARIZE_PHASE.SEARCHING_REFERENCES,
            ...refDetail,
          }),
      );
...
export function finalizeSummarizeOutput(
  raw,
  { isLecturer, referenceCatalog },
) {
  if (!isLecturer) return String(raw || "").trim();
  return finalizeLecturerReferences(String(raw || "").trim(), referenceCatalog)
    .markdown;
}
