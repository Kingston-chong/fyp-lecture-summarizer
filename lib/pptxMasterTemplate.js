  ...
  diagonal_flow(theme, W, H) {
    ...
    shapes.push(
      sp({
        ...
        name: "S2N_BottomBar",
        x: 0,
        y: H - Math.round(IN * 0.06),
        cx: W,
        cy: Math.round(IN * 0.06),
    ...
    shapes.push(
      sp({
        ...
        name: "S2N_DotL1",
        x: Math.round(IN * 0.3),
        y: Math.round(IN * 0.25),
        cx: Math.round(IN * 0.18),
        cy: Math.round(IN * 0.18),
        geom: "ellipse",
    ...
    shapes.push(
      sp({
        id: id++,
        name: "S2N_DotL2",
        x: Math.round(IN * 0.58),
        y: Math.round(IN * 0.15),
        cx: Math.round(IN * 0.1),
        cy: Math.round(IN * 0.1),
        geom: "ellipse",
        fill: solidFill(accH, 25),
      }),
    );
  ...
  botanical(theme, W, H) {
    ...
    shapes.push(
      sp({
        ...
        x: Math.round(W * 0.78),
        y: -Math.round(H * 0.25),
        cx: Math.round(W * 0.32),
        cy: Math.round(H * 0.65),
        geom: "ellipse",
        fill: solidFill(accH, 22),
      }),
    );
    ...
    shapes.push(
      sp({
        ...
        y: -Math.round(H * 0.1),
        cx: Math.round(W * 0.2),
        cy: Math.round(H * 0.45),
        geom: "ellipse",
        fill: solidFill(accLight, 30),
      }),
    );
    ...
    shapes.push(
      sp({
        ...
        name: "S2N_BottomBar",
        x: 0,
        y: H - Math.round(IN * 0.12),
        cx: Math.round(W * 0.45),
        cy: Math.round(IN * 0.07),
  ...
  geometric_grid(theme, W, H) {
    ...
    shapes.push(
      sp({
        id: id++,
        name: "S2N_TopBar",
        x: Math.round(IN * 0.22),
        y: 0,
        cx: W,
        cy: Math.round(IN * 0.065),
    ...
    for (let i = 0; i < 3; i++) {
      shapes.push(
        sp({
          id: id++,
          name: `S2N_Line${i}`,
          x: Math.round(W * 0.88) + i * Math.round(IN * 0.08),
          y: Math.round(H * 0.1),
          cx: Math.round(IN * 0.02),
          cy: Math.round(H * 0.5),
  ...
  gradient_wash(theme, W, H) {
    ...
    shapes.push(
      sp({
        ...
        x: Math.round(W * 0.55),
        y: -Math.round(H * 0.3),
        cx: Math.round(W * 0.65),
        cy: Math.round(H * 0.65),
        geom: "ellipse",
        fill: gradFill(accH, bgH, 0, 12, 0),
      }),
    );
    ...
    shapes.push(
      sp({
        ...
        name: "S2N_BottomGlow",
        x: 0,
        y: H - Math.round(IN * 0.08),
        cx: W,
        cy: Math.round(IN * 0.08),
    ...
    for (let row = 0; row < 3; row++) {
      for (let col = 0; col < 5; col++) {
        shapes.push(
          sp({
            id: id++,
            name: `S2N_Dot${row}_${col}`,
            x: Math.round(IN * (0.35 + col * 0.22)),
            y: Math.round(IN * (0.35 + row * 0.22)),
            cx: Math.round(IN * 0.055),
            cy: Math.round(IN * 0.055),
            geom: "ellipse",
            fill: solidFill(accH, 18),
          }),
        );
  ...
  warm_academic(theme, W, H) {
    ...
    shapes.push(
      sp({
        id: id++,
        name: "S2N_LeftRule",
        x: Math.round(IN * 0.55),
        y: Math.round(IN * 0.12),
        cx: Math.round(IN * 0.025),
        cy: Math.round(H * 0.7),
    ...
    shapes.push(
      sp({
        ...
        name: "S2N_BottomRule",
        x: Math.round(IN * 0.55),
        y: H - Math.round(IN * 0.25),
        cx: W - Math.round(IN * 0.55),
        cy: Math.round(IN * 0.025),
        fill: solidFill(accH, 40),
      }),
    );
    ...
    shapes.push(
      sp({
        id: id++,
        name: "S2N_RightAccent",
        x: W - Math.round(IN * 0.18),
        y: Math.round(IN * 0.12),
        cx: Math.round(IN * 0.18),
        cy: H - Math.round(IN * 0.12),
  ...
  minimal_edge(theme, W, H) {
    ...
    shapes.push(
      sp({
        id: id++,
        name: "S2N_RightLine",
        x: W - Math.round(IN * 0.015),
        y: Math.round(IN * 0.065),
        cx: Math.round(IN * 0.015),
        cy: H - Math.round(IN * 0.065),
        fill: solidFill(accH, 18),
      }),
    );
    ...
    shapes.push(
      sp({
        id: id++,
        name: "S2N_BottomLine",
        x: Math.round(IN * 0.18),
        y: H - Math.round(IN * 0.05),
        cx: W - Math.round(IN * 0.2),
        cy: Math.round(IN * 0.05),
...
function injectIntoSpTree(xml, shapesXml) {
  ...
  return xml.replace(/(<p:spTree>[\s\S]*?<\/p:grpSpPr>)/, `$1${shapesXml}`);
}
...
export async function applyMasterTemplate(sourceBuffer, theme, options = {}) {
  ...
  // Select template
  const templateKey =
    options.templateName && TEMPLATES[options.templateName]
