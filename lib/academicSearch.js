...
const MAX_QUERIES = 6;
/** Semantic Scholar / OpenAlex match better on 1–2 keywords than long phrases. */
const SEARCH_QUERY_MAX_WORDS = 2;
 ...
 * @param {string} token
 */
function isSignificantSearchToken(token) {
...
/**
 * Turn one phrase into 1–2 word Semantic Scholar queries.
 * @param {string} query
 * @returns {string[]}
 */
export function compactAcademicSearchQuery(query) {
  const raw = String(query || "")
    .trim()
    .replace(/\s+/g, " ");
  if (!raw) return [];

  const tokens = raw.split(/\s+/).filter(isSignificantSearchToken);
  if (tokens.length === 0) {
    return raw.length >= 3 ? [raw.slice(0, 40)] : [];
  }
  if (tokens.length === 1) return [tokens[0]];

  const out = [tokens.slice(0, SEARCH_QUERY_MAX_WORDS).join(" "), tokens[0]];
  if (tokens.length >= 2 && tokens[1] !== tokens[0]) {
    out.push(tokens[1]);
  }
  return out;
}
...
/**
 * Expand and dedupe queries into short keyword searches.
 * @param {string[]} queries
 * @param {number} [max]
 */
export function expandToShortSearchQueries(queries, max = MAX_QUERIES) {
  const seen = new Set();
  const out = [];
  for (const q of queries || []) {
    for (const short of compactAcademicSearchQuery(q)) {
      const trimmed = String(short || "")
        .trim()
        .slice(0, 60);
...
/**
 * Build short keyword queries from extracted lecture text and document names.
...
export function buildSearchQueries(combinedText, documents = []) {
  ...
  const names = (documents || [])
    ...
    .filter((n) => n.length > 3);
  if (names[0]) queries.push(...compactAcademicSearchQuery(names[0]));

  const topTerms = extractTopTerms(combinedText, 10);
  for (let i = 0; i < topTerms.length; i++) {
    queries.push(topTerms[i]);
    if (i + 1 < topTerms.length) {
      queries.push(`${topTerms[i]} ${topTerms[i + 1]}`);
    }
  }
  ...
  if (headingMatch?.[1]) {
    queries.push(...compactAcademicSearchQuery(headingMatch[1].trim()));
  }

  return expandToShortSearchQueries(queries, MAX_QUERIES);
}
...
export function mergeSearchQueries(primary, fallback, max = MAX_QUERIES) {
  return expandToShortSearchQueries(
    [...(primary || []), ...(fallback || [])],
    max,
  );
}
...
export async function resolveAcademicSearchPlan(
  ...
  queries = expandToShortSearchQueries(queries, MAX_QUERIES);
