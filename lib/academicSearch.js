...
export function mergeSearchQueries(primary, fallback, max = MAX_QUERIES) {
  ...
  for (const q of [...(primary || []), ...(fallback || [])]) {
    const trimmed = String(q || "").trim().slice(0, 120);
...
export function buildChatAcademicQueries(
  ...
  const seed = [userText, summaryTitle, summaryExcerpt].filter(Boolean).join("\n");
  const fromLecture = buildSearchQueries(seed, [{ name: summaryTitle }]);
  queries.push(...fromLecture.slice(0, Math.max(1, MAX_QUERIES - queries.length)));
...
export async function fetchRelatedPapersForChat({
  ...
  return filterPapersByRelevance(merged, relevanceSeed, {
    minScore,
    max: MAX_RESULTS,
  });
}
...
export async function fetchRelatedPapers(
  ...
  const merged = mergeAndDedupeResults(allHits);
  const relevant = filterPapersByRelevance(merged, combinedText, {
    terms: relevanceTerms,
  });
  return filterPapersByPublishedYear(relevant, yearRange);
}
