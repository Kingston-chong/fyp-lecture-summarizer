...
function patchSlideXml(slideXml, titleText, lines) {
  ...
  if (bodyPh && lines.length > 0) {
    ...
    patches.push({ start: bodyPh.start, end: bodyPh.end, replacement: patched });
  }

  if (titlePh && titleText) {
    const patched = replaceSpText(titlePh.xml, [titleText]);
    patches.push({ start: titlePh.start, end: titlePh.end, replacement: patched });
  }
 ...
 */
async function addNewSlide(zip, templateSlidePath, titleText, lines, notesText, presXml, presRelsXml) {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  const existingIds = [...presXml.matchAll(/<p:sldId\b[^>]*\bid="(\d+)"/g)].map((m) =>
    Number(m[1])
  );
    ...
    if (idx <= originalCount) {
      ...
      if (notesText) {
        ...
        if (notesRef && zip.file(notesRef.notesPath)) {
          const existingNotes = await zip.file(notesRef.notesPath).async("string");
