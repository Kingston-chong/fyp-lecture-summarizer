...
import { alaiFetch, getAlaiApiKeys } from "./alaiClient.js";
...
export async function getAlaiPptxUrlWithPoll(generationId, opts = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const { res, data } = await alaiFetch(`/generations/${generationId}`, {
      method: "GET",
    });
