...
/** Collect messages from an error chain (e.g. Google SDK wraps fetch errors). */
...
/**
 * True when trying the next Gemini slug/model may succeed (wrong model id,
 * quota / RPD / rate limits, overload). Not used for auth (401/403) or
 * obvious client mistakes we should not paper over.
 * @param {unknown} err
 */
function shouldRetryGeminiOnDifferentModel(err) {
  const status = Number(err?.status ?? err?.statusCode ?? err?.code);
  if (status === 401 || status === 403) return false;
  if (status === 429 || status === 503 || status === 404) return true;
  if (isOpenRouterModelNotFound(err)) return true;
...
function messageHasVisionParts(content) {
  ...
  return content.some(
    (p) =>
      p?.type === "image_url" &&
      String(p?.image_url?.url || "").startsWith("data:image/"),
  );
}
...
async function callOpenRouterChat(
  ...
  for (const slug of slugs) {
    try {
      ...
      lastErr = err;
      if (model === "gemini" && shouldRetryGeminiOnDifferentModel(err))
        continue;
      throw err;
    }
  }
  ...
  if (model === "gemini") {
    ...
    for (const modelName of uniqueCandidates) {
      try {
        ...
        lastErr = err;
        if (shouldRetryGeminiOnDifferentModel(err)) continue;
        throw err;
      }
    }
...
export async function runChat(
  ...
  const requestedMax = maxTokens ?? MAX_REPLY_TOKENS;
  ...
  const invoke = (sys, msgs, tokens) =>
    openrouterClient
      ? callOpenRouterChat(model, modelVariant, sys, msgs, tokens)
      : callDirectChat(model, modelVariant, sys, msgs, tokens);
  ...
  try {
    ...
    console.warn(
      "LLM context length exceeded; trimming input and retrying:",
      err?.message || err,
    );
    const retryMax = Math.max(256, Math.floor(budget.maxTokens * 0.4));
    const trimmed = trimChatPayload(
      systemPrompt,
      messages,
      retryMax,
    ...
    return await invoke(trimmed.systemPrompt, trimmed.messages, retryMax);
  }
}
