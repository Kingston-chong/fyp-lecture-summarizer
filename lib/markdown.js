...
/** No underscores — otherwise `_word_ … @@CITE_n@@` italic can swallow the placeholder. */
const CITE_PLACEHOLDER_RE = /@@CITE(\d{1,3})@@/g;
/** Legacy / corrupted placeholders after italic mangles `@@CITE_1@@`. */
const CITE_PLACEHOLDER_BROKEN_RE =
  /@@CITE(?:<\/?em>)?(\d{1,3})(?:<\/?em>)?@@/gi;

/**
 * Protect [n] citation markers from list parsers (chat bubbles).
 * @param {string} text
 */
function protectCitationMarkers(text) {
 ...
 * @param {string} html
 */
function restoreCitationMarkers(html) {
  return String(html || "")
    .replace(
      CITE_PLACEHOLDER_RE,
      (_full, n) => `<sup class="chat-cite-marker">[${n}]</sup>`,
    )
    .replace(CITE_PLACEHOLDER_BROKEN_RE, (_full, n) => {
      const num = String(n).replace(/<\/?em>/gi, "");
      return `<sup class="chat-cite-marker">[${num}]</sup>`;
    });
}

/**
 * @param {string} markdown
 * @param {{ chatMode?: boolean }} [options]
 */
export function markdownToHtml(markdown, options = {}) {
  ...
  if (chatMode) {
    text = protectCitationMarkers(text);
  }
  ...
  text = text.replace(/((?:^[ \t]*\d+[.)]\s+.+\n?)+)/gm, (block) => {
    ...
    const startAttr =
      firstNumMatch && firstNumMatch[1] !== "1"
        ? ` start="${firstNumMatch[1]}"`
        : "";
    const items = lines
      .map((line) => line.replace(/^[ \t]*\d+[.)]\s+(.+)$/, "<li>$1</li>"))
      .filter((x) => x.startsWith("<li>"));
    return items.length ? `<ol${startAttr}>${items.join("")}</ol>` : block;
  });
  ...
  if (chatMode) {
    html = restoreCitationMarkers(html);
  }

  return html;
}

/** Chat bubble markdown — preserves [n] citation markers in display. */
export function chatMarkdownToHtml(markdown) {
  return markdownToHtml(markdown, { chatMode: true });
}
