...
function appLogoUrlForEmail() {
  const base = (
    process.env.NEXT_PUBLIC_APP_URL ||
    process.env.NEXTAUTH_URL ||
    ""
  ).replace(/\/$/, "");
  return base ? `${base}/icon.png` : "";
}

export function emailBrandedHeaderHtml() {
  const logoUrl = appLogoUrlForEmail();
  const logoCell = logoUrl
    ? `<img src="${logoUrl}" width="34" height="34" alt="Slide2Notes" style="display:block;border-radius:9px;width:34px;height:34px;" />`
