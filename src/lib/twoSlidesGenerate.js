...
function authHeaders() {
  ...
  if (!key) throw new Error("TWOSLIDES_API_KEY is not configured on the server.");
  return {
...
export async function generatePptxWithTwoSlides({
  ...
  if (directUrl) {
    const fileRes = await fetch(directUrl, { signal: AbortSignal.timeout(60_000) });
    if (!fileRes.ok) {
  ...
  if (!jobId) throw new Error("2slides: no downloadUrl/jobId returned from generate");

  const t0 = Date.now();
  while (Date.now() - t0 < pollTimeoutMs) {
    await sleep(1500);
    const check = await fetchJson(`${TWOSLIDES_BASE}/api/v1/jobs/${encodeURIComponent(String(jobId))}`, {
      method: "GET",
      headers: { Authorization: authHeaders().Authorization },
      signal: AbortSignal.timeout(20_000),
    });
    const status = String(check?.data?.status || check?.status || "").toLowerCase();
    const url = extractDownloadUrl(check);
    if (url && (status === "success" || status === "completed" || status === "done" || status === "")) {
      const fileRes = await fetch(url, { signal: AbortSignal.timeout(60_000) });
      if (!fileRes.ok) throw new Error(`2slides: download failed (${fileRes.status})`);
      const ab = await fileRes.arrayBuffer();
      return Buffer.from(ab);
    }
    if (status === "failed" || status === "error") {
      throw new Error(check?.data?.error || check?.error || "2slides: job failed");
    }
  }
 ...
 */
export function improvedSlidesToTwoSlidesUserInput({ title, subtitle, slides, references }) {
  const lines = [];
  ...
  for (const s of Array.isArray(slides) ? slides : []) {
    ...
    const heading = String(s?.title || (Number.isFinite(idx) ? `Slide ${idx}` : "Slide")).trim();
