...
function run() {
  const complexPromptLikeSpec = {
    ...
    cover: {
      shapes: [{ type: "rect", x: 0, y: 0, w: "100%", h: "100%", fill: "#0b2f22" }],
      title: { x: 0.5, y: 1.8, w: 8.5, h: 1.2 },
      subtitle: { x: 0.5, y: 3.4, w: 8.5, h: 0.8 },
    },
    content: {
      shapes: [{ type: "rect", x: 0, y: 0, w: "100%", h: "100%", fill: "#0f3f2b" }],
  ...
  assert(normalized.spec.content.body.h <= 1, "body height should be clamped into normalized bounds");
  assert(normalized.spec.cover.shapes[0].w <= 2, "shape width should be finite and clamped");
  ...
  const resolvedSpec = {
    cover: { shapes: [], title: { x: 0.08, y: 0.24, w: 0.74, h: 0.34 }, subtitle: { x: 0.08, y: 0.60, w: 0.7, h: 0.18 } },
    content: { shapes: [], title: { x: 0.07, y: 0.10, w: 0.88, h: 0.16 }, body: { x: 0.07, y: 0.27, w: 0.88, h: 0.64 } },
  };
