...
function shouldRetryGeminiOnDifferentModel(err) {
  ...
  if (msg.includes("resource_exhausted") || msg.includes("resource has been exhausted"))
    return true;
  if (msg.includes("quota") && (msg.includes("exceed") || msg.includes("exhausted") || msg.includes("limit")))
    return true;
  if (msg.includes("rate limit") || msg.includes("too many requests")) return true;
  if (msg.includes("overloaded") || msg.includes("capacity")) return true;
  if (msg.includes("per day") || msg.includes("daily limit") || msg.includes("requests per day"))
    return true;
  if (msg.includes(" 429 ") || msg.startsWith("429") || msg.includes("status: 429")) return true;
  return false;
}
...
async function callOpenRouterChat(model, modelVariant, systemPrompt, messages, maxTokens) {
  ...
  for (const slug of slugs) {
    try {
      ...
      lastErr = err;
      if (model === "gemini" && shouldRetryGeminiOnDifferentModel(err)) continue;
      throw err;
    }
  }
